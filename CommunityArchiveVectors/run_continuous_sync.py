#!/usr/bin/env python3
"""
Continuous sync script - runs 10K tweet batches sequentially until complete.
...
def run_batch(batch_num):
    """Run a single 10K batch and return True if successful."""
    print(f"\n{'='*80}")
...
def main():
    ...
    print(f"📊 Each batch processes 10K tweets (~10 mins per batch)")

    if max_batches:
        print(f"🎯 Target: {max_batches} batches (~{max_batches * 10000:,} tweets)")
    else:
